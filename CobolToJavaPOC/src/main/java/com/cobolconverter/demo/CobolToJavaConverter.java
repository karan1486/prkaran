...
public class CobolToJavaConverter {
	...
	private static void writeHeader(BufferedWriter bw, RPVI3053 rpvi3053) throws IOException {
		...
		header.append("02010398101");
		rpvi3053.setORB_ITEM_NUM(0);
		header.append(rpvi3053.getORB_ITEM_NUM());
		rpvi3053.setORB_LRECL("080");
		header.append(rpvi3053.getORB_LRECL());
		rpvi3053.setORB_BLKSIZE("0800");
		header.append(rpvi3053.getORB_BLKSIZE());
		bw.write(header.toString());
		bw.newLine();
	}
	...
	private static void writeDetail(BufferedWriter bw, RPVI3053 rpvi3053) throws IOException {
		...
		sb.append(rpvi3053.getORB_AMT_PAID().toString().replace(".", ""));
	...
	private static void writeTrailer(BufferedWriter bw, RPVI3053 rpvi3053) throws IOException {
		...
		tailer.append(WS_TOTAL_CNT);
		rpvi3053.setORB_TAPE_AMT(WS_TOTAL_AMT);
		tailer.append(String.format("%.2f", rpvi3053.getORB_TAPE_AMT()).replace(".", ""));
		bw.write(tailer.toString());
		WS_TOTAL_CNT++;
		tailer.append(rpvi3053.getORB_ITEM_NUM());
		tailer.append("02010398101");
		rpvi3053.setORB_BANK_ID("02010398101");
		rpvi3053.setORB_BANK_ACCT_NUM("02010398101");
		WS_TOTAL_CNT++;
		tailer.append(sdf.format(c1.getTime()));
	...
	private static RPVI3053 convertRPVI3053(String sCurrentLine) {
		...
		cpRPVI3053.setORB_TRACK_NUM_PRI(sCurrentLine.substring(100, 123));
		cpRPVI3053.setORB_TRACK_NUM_SEC(sCurrentLine.substring(123, 128));

		cpRPVI3053.setORB_PMT_TYPE(sCurrentLine.substring(128, 130));
		cpRPVI3053.setORB_NSF_INDR(sCurrentLine.substring(130, 131));
		cpRPVI3053.setORB_NSF_REASON_CODE(sCurrentLine.substring(131, 134));
		cpRPVI3053.setORB_ProcessType(sCurrentLine.substring(134, 139));
		cpRPVI3053.setORB_PMT_CONFIRMATION_NO(sCurrentLine.substring(139, 151));
	...
	private static void writeVZPPTotals(BufferedWriter bwvzppTot, RPVIPKEY rpvipkey) throws IOException {
		...
		rpvi8001.setVZPP_PAYMENT_AMOUNT((short) WS_VZPP_ORB_PMT_AMT);
		vzppTot.append(String.format("%.2f", WS_VZPP_ORB_PMT_AMT).replace(".", ""));
		rpvi8001.setVZPP_PAYMENT_ITEMS((short)WS_VZPP_ORB_PMT_ITEMS);
		vzppTot.append(decimalFormat.format(WS_VZPP_ORB_PMT_ITEMS).replace(".", ""));
		rpvi8001.setVZPP_RETURN_AMOUNT((short) WS_VZPP_ORB_RTN_AMT);

		vzppTot.append(String.format("%.2f", WS_VZPP_ORB_RTN_AMT).replace(".", ""));
		rpvi8001.setVZPP_RETURN_ITEMS((short) WS_VZPP_ORB_RTN_ITEMS);
